...
class YashController(KesslerController):
        
    def __init__(self, chromosome=None):
        self.eval_frames = 0 #What is this?

        # Extract values from the chromosome
        (bullet_time_s,
        bullet_time_m,
        bullet_time_l,
        theta_delta_nl,
        ...
        ship_thrust_z,
        ship_thrust_ps,
        ship_thrust_pl) = chromosome[0].value
        ...
        bullet_time['S'] = fuzz.trimf(bullet_time.universe,[0,0,0.05*bullet_time_s])
        bullet_time['M'] = fuzz.trimf(bullet_time.universe, [0,0.05*bullet_time_m,0.1])
        ...
        ship_turn['NL'] = fuzz.trimf(ship_turn.universe, [-180,-180+ship_turn_nl,-120])
        ship_turn['NS'] = fuzz.trimf(ship_turn.universe, [-120,-90+ship_turn_ns,-60])
        ship_turn['Z'] = fuzz.trimf(ship_turn.universe, [-60,0+ship_turn_z,60])
        ship_turn['PS'] = fuzz.trimf(ship_turn.universe, [60,90+ship_turn_ps,120])
        ship_turn['PL'] = fuzz.trimf(ship_turn.universe, [120,180+ship_turn_pl,180])
        
        #Declare singleton fuzzy sets for the ship_fire consequent; -1 -> don't fire, +1 -> fire; this will be  thresholded
        #   and returned as the boolean 'fire'
        ship_fire['N'] = fuzz.trimf(ship_fire.universe, [-1,-1,0.0+ship_fire_n])
        ship_fire['Y'] = fuzz.trimf(ship_fire.universe, [0.0+ship_fire_y,1,1]) 
        ...
        rule7 = ctrl.Rule(bullet_time['M'] & theta_delta['NS'], (ship_turn['NS'], ship_fire['N']))
        rule8 = ctrl.Rule(bullet_time['M'] & theta_delta['Z'], (ship_turn['Z'], ship_fire['Y']))    
        rule9 = ctrl.Rule(bullet_time['M'] & theta_delta['PS'], (ship_turn['PS'], ship_fire['N']))
        ...
        rel_ast_dist['NS'] = fuzz.trimf(rel_ast_dist.universe, [50,150+rel_ast_dist_ns,275])
        rel_ast_dist['Z'] = fuzz.trimf(rel_ast_dist.universe, [225,350+rel_ast_dist_z,475])
        rel_ast_dist['PS'] = fuzz.trimf(rel_ast_dist.universe, [425,550+rel_ast_dist_ps,650])
        rel_ast_dist['PL'] = fuzz.smf(rel_ast_dist.universe,600,700)

        rel_ast_angl['NL'] = fuzz.trimf(rel_ast_angl.universe, [-180,-180+rel_ast_angl_nl,-120])
        rel_ast_angl['NS'] = fuzz.trimf(rel_ast_angl.universe, [-120,-90+rel_ast_angl_ns,-60])
        rel_ast_angl['Z'] = fuzz.trimf(rel_ast_angl.universe, [-60,0+rel_ast_angl_z,60])
        rel_ast_angl['PS'] = fuzz.trimf(rel_ast_angl.universe, [60,90+rel_ast_angl_ps,120])
        rel_ast_angl['PL'] = fuzz.trimf(rel_ast_angl.universe, [120,180+rel_ast_angl_pl,180])
          
        ship_thrust['NL'] = fuzz.zmf(ship_thrust.universe,0,50)
        ship_thrust['NS'] = fuzz.trimf(ship_thrust.universe, [25,50+ship_thrust_ns,75])
        ship_thrust['Z'] = fuzz.trimf(ship_thrust.universe, [50,75+ship_thrust_z,100])
        ship_thrust['PS'] = fuzz.trimf(ship_thrust.universe, [75,100+ship_thrust_ps,125])
